...
;;@end example
(define-function (equal? obj1 obj2)
               ...
               (labels ((loop (dims scales)
                              (if (null? dims)
                                  (array$construct dimensions
                                                    ...
                                                    store )
                                  (loop (cdr dims)
                                        (cons (* (car dims) (car scales))
                                              scales ) ))))
                 (loop (reverse dimensions) '(1))) ))))))
;;@args prototype k1 @dots{}
;;@0 is an alias for @code{make-array}.
(define-function create-array #'make-array)
...
;;
;;@example
;;(define fred (make-array '#(NIL) 8 8))
...
;;                @result{} #0A 3
;;@end example
(define-function (list->array rank proto lst)
  (cl:let (dimensions)
    (setq dimensions
...
;;                @result{} ho
;;@end example
(define-function (array->list ra)
  (labels ((ra2l (dims idxs)
             (if (null? dims)
...
(define-function (make-prototype-checker name pred? creator)
  (lambda args
    (case (length args)
      ((1) (if (funcall pred? (car args))
	       (funcall creator (car args))
	       (error name 'incompatible 'type (car args))))
      ((0) (funcall creator))
      (otherwise (error name 'wrong 'number 'of 'args args)))))

(define-function (integer-bytes?? n)
  (lambda (obj)
    (and (integer? obj)
	 (exact? obj)
